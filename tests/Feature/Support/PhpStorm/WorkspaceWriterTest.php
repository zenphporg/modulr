...
test('it can be instantiated', function (): void {
  ...
  $writer = new WorkspaceWriter('/tmp/test.xml', $registry);
  expect($writer)->toBeInstanceOf(WorkspaceWriter::class);
});

test('it extends config writer', function (): void {
  $reflection = new ReflectionClass(WorkspaceWriter::class);
...
test('it throws exception when file cannot be read', function (): void {
  $registry = $this->app->make(Registry::class);
  $writer = new WorkspaceWriter('/non/existent/path.xml', $registry);
...
test('it throws exception when xml cannot be parsed', function (): void {
  ...
  try {
    $registry = $this->app->make(Registry::class);
    $writer = new WorkspaceWriter($tempFile, $registry);
...
test('it can write config with valid xml file', function (): void {
  ...
  $registry = $this->app->make(Registry::class);
  $writer = new WorkspaceWriter($config_path, $registry);
